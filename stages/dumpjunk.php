	...
	public function extractJunk($bookID) {
		...
		foreach($allXhtmlFiles as $xhtmlFile){
			...
			foreach($finalWords as $word){
				...
				if($this->checkTiriTiji($word))
					array_push($tempTiriTiji, $word);
			}
			...
			if($tempTiriTiji){
				...
				array_push($tiriTijiWord, $xhtmlFile);
				array_push($tiriTijiWord, "List of తీరి & తీజి  words");
				$tiriTijiWord = array_merge($tiriTijiWord,$tempTiriTiji);
			}
		}
		...
		if($junkWords || $falseWord || $tiriTijiWord)
			file_put_contents(RAW_SRC . $bookID . '/' . $bookID . ".junk.txt", implode("\n",array_merge($junkWords, $falseWord, $tiriTijiWord)));

	}

	public function checkTiriTiji($word) {

		if(preg_match('/తీరి|తీజి/', $word)) return 1;
		return 0;
	}
