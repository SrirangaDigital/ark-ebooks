...
class Stages{
	...
	public function processRawHTML($text) {
		...
		$text = preg_replace('/<span .*font.*Arial.*>/i', '<span-class="en">', $text);
		$text = preg_replace('/<span .*font.*Bookman Old Style.*>/i', '<span-class="en">', $text);
		...
		$text = preg_replace('/<\/i>/i', '</em>', $text);
		$text = preg_replace('/<spanstyle=/', '<span style=', $text);
