...
class Stages{
	...
	public function process($bookID,$file) {
		 $baseFileName = basename($file); //manu
		 $stage2File = str_replace('Stage1', 'Stage2', $file); //manu
		
		 if(!file_exists($stage2File)) {  //manu
		...
		file_put_contents($fileName, $processedHTML);
			 ...
			 $processedHTML = file_get_contents($stage2File);  //manu
			 $processedHTML = preg_replace('/<SPAN>/', '', $processedHTML); //manu
		 }
	...
	public function processRawHTML($text) {
		...
		$text = preg_replace_callback('/(<P [^\n]*?margin-left:(\d+)px[^\n]*?>)(.*?)<\/P>/is',
			function($matches) {

				if(intval($matches[2] < 30))
					return '<p>' . "$matches[3]" . '</p>';
				elseif(intval($matches[2] < 150))
					return '<pvt>' . "$matches[3]" . '</pvt>';
				else
					return '<ptr>' . "$matches[3]" . '</ptr>';
			},
			$text);
		
		$text = preg_replace('/(<P [^\n]*?text-align:left[^\n]*?>)(.*?)<\/P>/is', '<p>' . "$2" . '</p>', $text);
		$text = preg_replace('/(<P [^\n]*?text-align:justify[^\n]*?>)(.*?)<\/P>/is', '<p>' . "$2" . '</p>', $text);
		$text = preg_replace('/(<P style[^\n]*>)(.*?)<\/P>/is', '<ptc>' . "$2" . '</ptc>', $text);

		// Special cases that need to be retained
		$text = preg_replace('/(<SPAN [^\n]*?font-weight:[ ]*bold[^\n]*?>)(.*?)<\/span[1]*>/is', "$1" . '<strong>' . "$2" . '</strong>' . "</span1>", $text);
		$text = preg_replace('/(<SPAN [^\n]*?font-style:[ ]*italic[^\n]*?>)(.*?)<\/span[1]*>/is', "$1" . '<em>' . "$2" . '</em>' . "</span1>", $text);
		$text = preg_replace('/(<SPAN [^\n]*?font[^\n]*?(dingb)[^\n]*?>)(.*?)<\/span[1]*>/is', "$1" . '<spanDing>' . "$3" . '</spanDing>' . "</span1>", $text);
		$text = preg_replace('/(<SPAN [^\n]*?font[^\n]*?(times|Sylfaen|Tahoma|Helvetica|Calisto MT|Century Schoolbook|Arial)[^\n]*?>)(.*?)<\/span[1]*>/is', "$1" . '<spanEn>' . "$3" . '</spanEn>' . "</span1>", $text);
		
		$text = preg_replace('/(<SPAN style.*?>)/i', '', $text);
		$text = preg_replace('/(<\/span>)/i', '', $text);
		$text = preg_replace('/(<\/span1>)/i', '', $text);
		
		// General cases that need to be deleted like <p style
		$text = preg_replace('/<([a-zA-Z0-9]+) .*\/>/i', "<$1-/>", $text);
		$text = preg_replace('/<([a-zA-Z0-9]+) .*>/i', "<$1>", $text);

		$text = preg_replace('/<ptc>/i', '<p class="text-center">', $text);
		$text = preg_replace('/<\/ptc>/i', '</p>', $text);
		$text = preg_replace('/<ptr>/i', '<p class="text-right">', $text);
		$text = preg_replace('/<\/ptr>/i', '</p>', $text);
		$text = preg_replace('/<pvt>/i', '<p class="verse-text">', $text);
		$text = preg_replace('/<\/pvt>/i', '</p>', $text);
		...
		$text = str_replace("</DIV>", "</SECTION>", $text);
		$text = preg_replace("/(<(P|LI|DIV|H\d))/i", "\n$1", $text);
		$text = preg_replace("/(<[\/]*(UL|OL|SECTION|BODY|HTML)>)/i", "\n$1", $text);

		$text = str_replace('</spanEn><spanEn>', '', $text);
		$text = str_replace('</spanDing><spanDing>', '', $text);
		$text = str_replace('</strong><strong>', '', $text);
		$text = str_replace('</em><em>', '', $text);
		$text = str_replace('<spanEn>', '<span class="en">', $text);
		$text = str_replace('<spanDing>', '<span class="ding">', $text);
		$text = str_replace('</spanEn>', '</span>', $text);
		$text = str_replace('</spanDing>', '</span>', $text);

		// Remove head items
		$text = preg_replace("/<(STYLE|META|HEAD).*\n/i", "", $text);
		$text = preg_replace("/<\/(STYLE|META|HEAD).*\n/i", "", $text);
		
		// Club consecutive verse texts
		$text = preg_replace('/<p class="verse-text">(.*)<\/p>\n<p class="verse-text">(.*)<\/p>/', '<p class="verse-text">' . "$1" . '<br />' . "$2" . '</p>', $text);
		$text = preg_replace('/<p class="verse-text">(.*)<\/p>\n<p class="verse-text">(.*)<\/p>/', '<p class="verse-text">' . "$1" . '<br />' . "$2" . '</p>', $text);
		$text = preg_replace('/<p class="verse-text">(.*)<\/p>\n<p class="verse-text">(.*)<\/p>/', '<p class="verse-text">' . "$1" . '<br />' . "$2" . '</p>', $text);
		$text = preg_replace('/<p class="verse-text">(.*)<\/p>\n<p class="verse-text">(.*)<\/p>/', '<p class="verse-text">' . "$1" . '<br />' . "$2" . '</p>', $text);
		$text = preg_replace('/<p class="verse-text">(.*)<\/p>\n<p class="verse-text">(.*)<\/p>/', '<p class="verse-text">' . "$1" . '<br />' . "$2" . '</p>', $text);
	...
	public function anu2Unicode ($text) {
		...
		$text = preg_replace('/([0-9a-zA-Z [:punct:]]+)/', '<span class="en">' . "$1" . '</span>', $text);
		$text = preg_replace('/<span class="en">([ [:punct:]]+)<\/span>/', "$1", $text);
		$text = preg_replace('/<span class="en">([\h[:punct:]]*)(.*?)(\h*)<\/span>/', "$1" . '<span class="en">' . "$2" . '</span>' . "$3", $text);
